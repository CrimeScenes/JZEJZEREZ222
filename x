...
-- Function to tween camera with specified easing
local function TweenToPosition(targetPosition)
    local cameraCFrame = Camera.CFrame
    local targetCFrame = CFrame.new(cameraCFrame.Position, targetPosition)

    -- Create tween info for easing
    local tweenInfo = TweenInfo.new(
        getgenv().CamLock.Normal.Smoothness, -- Duration of tween based on smoothness
        Enum.EasingStyle[getgenv().CamLock.EasingStyle], -- Easing style
        Enum.EasingDirection.Out -- Easing direction
    )
    
    -- Create and play the tween for camera movement
    local tween = TweenService:Create(Camera, tweenInfo, {CFrame = targetCFrame})
    tween:Play()
end

-- Main RenderStepped function for targeting and camera locking
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        -- Stop targeting if target health falls below threshold
        ...
        if BodyPart then
            ...
            if predictedPosition then
                -- Smooth movement if SmoothnessEnabled is on, else set directly
                if getgenv().CamLock.Normal.SmoothnessEnabled then
                    TweenToPosition(predictedPosition)
                else
        ...
        -- Silent shooting functionality for locked targets
